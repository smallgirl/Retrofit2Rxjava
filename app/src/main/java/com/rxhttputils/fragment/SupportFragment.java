...
/**
 * Created by wanghaitao on 2018/4/20.
 */
...
public class SupportFragment extends BaseFragment {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_delay:
                Observable.just(1, 2, 3).delay(2, TimeUnit.SECONDS).observeOn(AndroidSchedulers.mainThread()).subscribe(consumer);
                break;
            //do操作符,其下细分有很多内容，其作用就是为源Observable对象提供回调
            case R.id.btn_do:
                Observable.range(0, 3).doOnNext(consumer).subscribe(consumer);
                break;
            //subscribeOn操作符，指定subscribe()所发生的线程，即Observable.OnSubscribe被激活时所处的线程。或者叫做事件产生的线程
            case R.id.btn_subscribeOn:
                Observable.just("当前线程的ID为：" + Thread.currentThread().getName()).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()).subscribe(consumer);
                break;
            //observeOn操作符,指定Subscriber所运行的线程。或者叫做事件消费的线程
            case R.id.btn_observeOn:
                Observable.just("当前的线程ID为：" + Thread.currentThread().getName()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(consumer);
                break;
